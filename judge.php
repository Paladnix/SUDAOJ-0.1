...
class Judge
{
    ...
    function Judge($pid, $ID, $code, $comp, $in, $out, $time, $memory, $compile_out, $user_out)
    {
        $Pid = $pid;
        $RunID = $ID;
        $Code = $code;
    ...
    function Run()
    {
        ...
        if( $Compiler == "g++" )
        {
            ...
            $sh = "sudo su judge ./Run_C.sh $IN $Command $Exe $User_out$RunID.out $Time $Memory";
        }
        else if( $Compiler == "javac")
        {
            $Command = "java";

            $sh = "sudo su judge ./Run_Java.sh $IN $Command $Exe $User_out$RunID.out $Time $Memory";
        }
        else if( $Compiler == "python3" )
        {
            $Command = "python3";

            $sh = "sudo su judge ./Run_Py.sh $IN $Command $Exe $User_out$RunID.out $Time $Memory";
        }
